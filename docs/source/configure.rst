...
If you installed the EditKit software using the Ubuntu package then an
...
If you're doing development or testing, you can run ``editkit-manage runserver``
...
If you installed EditKit manually then you'll need to copy the
wsgi template file and edit it::

  mkdir deploy
  cp install_config/editkit_virtualenv.wsgi.template deploy/editkit.wsgi

Then open up ``editkit.wsgi`` and set ``VIRTUAL_ENV_PATH`` to the absolute
path to the virtualenv you installed EditKit in.
...
``WSGIScriptAlias / /path/to/your/deploy/editkit.wsgi`` -- you'll need to
change ``/path/to/your/deploy/`` to be the path to where the ``editkit.wsgi``
file lives.

In all of the ``Alias ...`` lines you'll need to change
``/path/to/your/env`` to be the absolute path to the virtualenv where
you installed EditKit.
